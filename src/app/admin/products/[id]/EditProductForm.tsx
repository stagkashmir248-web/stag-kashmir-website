...
export default function EditProductForm({ product }: { product: ProductWithVariations }) {
    ...
    const [variations, setVariations] = useState<VariationRow[]>(
        product.variations.map(v => {
            ...
            return { id: v.id, size: parts[0] || "", weight: parts[1] || "", extra: parts[2] || "", price: v.price.toString(), compareAtPrice: (v as any).compareAtPrice?.toString() || "", stock: v.stock.toString() };
        })
    );

    const [willowType, setWillowType] = useState((product as any).willowType || "");
    const [grade, setGrade] = useState((product as any).grade || "");
    const [blade, setBlade] = useState((product as any).blade || "");
    const [ballType, setBallType] = useState((product as any).ballType || "");
    const [warranty, setWarranty] = useState((product as any).warranty || "");
    const [features, setFeatures] = useState<string[]>((product as any).features || []);
    ...
    const [customSpecs, setCustomSpecs] = useState<CustomSpec[]>(Array.isArray((product as any).customSpecs) ? (product as any).customSpecs : []);
    ...
    const [genDefaultPrice, setGenDefaultPrice] = useState(price || "0");
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <SectionCard icon="tune" title="Technical Specs & Category" subtitle="Displayed in the Tech Specs card on the product page">
                ...
                <div className="border-t border-slate-700/50 pt-5 px-6 pb-6">
                    <div className="flex items-center justify-between mb-3">
                        <label className="block text-sm font-semibold text-slate-200 mb-1.5">Custom Technical Specs <span className="text-slate-600 font-normal normal-case">— extra rows in the specs table on the product page</span></label>
